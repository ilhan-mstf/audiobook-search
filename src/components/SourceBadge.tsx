...
export default function SourceBadge({ source }: Props) {
  ...
  const colorClass = source.isFree || isLibrary
    ? isLibrary
  ...
  return (
    <a
      ...
      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors ${colorClass}`}
    >
      <span>{label}</span>
